...
4. Codex、Claude Code 或项目级安装，优先运行仓库提供的安装器：
   ...
   ```bash
   python3 scripts/安装Skill.py --agent <codex|claude|project>
   ```
...
5. 先运行 `node scripts/检查运行环境.cjs` 检查实际运行时。需要隔离安装时，确认网络下载并给安装器增加 `--with-runtime`；它会安装 Playwright、Chromium 和 PyMuPDF。Node.js 至少 24，Python 至少 3.10。不要修改其他项目的依赖。
...
9. 验证 Skill：确认目标目录存在 `anqian-dongcha-baogao/SKILL.md`；运行 `node --test tests/*.test.cjs`；按仓库安装文档在临时目录生成匿名报告、完成浏览器验收和 PDF 检查。测试文件不能写进我的业务项目。技术通过不代表新报告图表比例或业务内容通过。
