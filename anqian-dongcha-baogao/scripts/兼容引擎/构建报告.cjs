...
const {renderDiagram,validateDiagramPage}=require('./可重排图形.cjs');
const skill=path.resolve(__dirname,'../..');
const templates=path.join(skill,'assets/兼容模板');
...
function build(project,overwrite=false){
  ...
  function block(item){
    if(item.type==='diagram')return renderDiagram(item);
  ...
  const pageHtml=config.pages.map((page,index)=>{
    ...
    const diagramPage=validateDiagramPage(page);hasDiagrams=hasDiagrams||diagramPage;
    ...
    return `<section class="page${diagramPage?' diagram-page':''}" id="p${index+1}" data-page-id="${escape(page.id)}"><div class="mast"><span>${escape(config.publisher)} <b>研究与洞察</b></span><span>${escape(config.title)} · ${escape(page.section)}</span></div><header><div class="eyebrow">${String(index+1).padStart(2,'0')} / ${escape(page.section)}</div><h1>${escape(page.title)}</h1><p>${escape(page.subtitle||'')}</p></header><div class="body ${page.layout}${diagramPage?' diagram-body':''}">${body}</div><div class="takeaway"><b>我们的观点</b><span>${escape(page.takeaway)}</span></div><footer><div>${links}${links?'<br>':''}${escape(page.sourceNote||'')} · ${escape(config.date)}</div><b>${index+1} / ${config.pages.length}</b></footer></section>`;
  }).join('\n');
  ...
  const diagramStyle=hasDiagrams?fs.readFileSync(path.join(__dirname,'可重排图形.css'),'utf8'):'';
  const controlStyle=config.pages.some(page=>page.blocks.some(item=>item.type==='diagram'&&item.variant==='control-chain'))?fs.readFileSync(path.join(__dirname,'控制链图形.css'),'utf8'):'';
  const mapStyle=config.pages.some(page=>page.blocks.some(item=>item.type==='diagram'&&item.variant==='relationship-map'))?fs.readFileSync(path.join(__dirname,'关系图.css'),'utf8'):'';
  const structuredStyle=config.pages.some(page=>page.blocks.some(item=>item.type==='diagram'&&['categorical-matrix','equation-flow'].includes(item.variant)))?fs.readFileSync(path.join(__dirname,'矩阵算式.css'),'utf8'):'';
  const replacements={TITLE:escape(config.title),STYLE:fs.readFileSync(path.join(templates,'报告样式.css'),'utf8')+extraStyle+diagramStyle+controlStyle+mapStyle+structuredStyle,PAGES:pageHtml,ATTACHMENTS:payload.join('\n'),SCRIPTS:scripts.join('\n')};
