...
def main():
    ...
    parser.add_argument("--delivery", type=Path, help="交付目录，默认项目内的交付")
    parser.add_argument("--checks", type=Path, help="验收目录，默认项目内的验收")
    ...
    delivery = (root / (args.delivery or "交付")).resolve()
    out = (root / (args.checks or "验收")).resolve()
    out.mkdir(parents=True, exist_ok=True)
    (out / "PDF验收结果.json").write_text(json.dumps({"status": "running"}))
    browser = json.loads((out / "验收结果.json").read_text())
    manifest = json.loads((delivery / "成品清单.json").read_text())
    digest = lambda file: hashlib.sha256(file.read_bytes()).hexdigest()
    assert browser["status"] == "passed", "需先通过当前报告的浏览器验收"
    assert digest(delivery / "案前洞察.html") == browser["htmlSha256"] == manifest["htmlSha256"], "HTML与浏览器验收版本不一致"
    assert digest(delivery / "案前洞察.pdf") == browser["pdf"]["sha256"], "PDF与浏览器导出版本不一致"
    pdf = fitz.open(delivery / "案前洞察.pdf")
